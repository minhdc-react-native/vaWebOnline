...
function VcToolbar({ children }: { children?: ReactNode }) {
  return (
...
function VcToolbarActions({ children }: { children?: ReactNode }) {
  return <div className="flex items-center gap-2.5">{children}</div>;
}

function VcToolbarBreadcrumbs() {
...
function VcToolbarHeading({ title = '', description }: ToolbarHeadingProps) {
  const { pathname } = useLocation();
...
export { VcToolbar, VcToolbarActions, VcToolbarBreadcrumbs, VcToolbarHeading };
